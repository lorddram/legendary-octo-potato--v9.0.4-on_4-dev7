...
def cmd_set(node: dict, addr: str, field: str, rest: str):
    ...
    if field == "txt":
        ...
        if kind != "synthesis":
            print("Type only valid in Synthesis. Use 'mark' here."); return
        if val == "-" or val == "":
            cell["t"] = None; add_history(cell, "clear type")
        elif val in ("D","R","A","M"):
            cell["t"] = val; add_history(cell, f"set type {val}")
        else:
            print("Type must be D,R,A,M or -")
    else:
        print("Unknown field.")
